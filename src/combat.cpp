...
bool Combat::isPlayerCombat(const Creature* target)
{
	...
	Creature* targetMaster = target->getMaster();
...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	...
	if (const Player* targetPlayer = target->getPlayer()) {
		...
		if (const Player* attackerPlayer = attacker->getPlayer()) {
			...
			const Tile* attackerPlayerTile = attackerPlayer->getTile();
			if (attackerPlayerTile && attackerPlayerTile->hasFlag(TILESTATE_NOPVPZONE) &&
	...
	if (attacker->isSummon()) {
		Creature* attackerMaster = attacker->getMaster();
		if (attackerMaster) {
			if (const Player* masterAttackerPlayer = attackerMaster->getPlayer()) {
				...
				const Tile* masterTargetTile = targetPlayer->getTile();
				if (masterTargetTile && masterTargetTile->hasFlag(TILESTATE_NOPVPZONE)) {
					return RETURNVALUE_ACTIONNOTPERMITTEDINANOPVPZONE;
				}
		...
		if (const Player* attackerPlayer = attacker->getPlayer()) {
			...
			Creature* targetSummonMaster = target->getMaster();
			if (target->isSummon() && targetSummonMaster && targetSummonMaster->getPlayer() && target->getZone() == ZONE_NOPVP) {
				return RETURNVALUE_ACTIONNOTPERMITTEDINANOPVPZONE;
			}
	...
	if (g_game.getWorldType() == WORLD_TYPE_NO_PVP) {
		Creature* attackerMaster = attacker->getMaster();
		if (attacker->getPlayer() || (attacker->isSummon() && attackerMaster && attackerMaster->getPlayer())) {
			if (target->getPlayer()) {
...
void Combat::combatTileEffects(const SpectatorVec& spectators, Creature* caster, Tile* tile, const CombatParams& params)
{
	...
	if (caster) {
		Player* casterPlayer;
		if (caster->isSummon()) {
			Creature* casterMaster = caster->getMaster();
...
void MagicField::onStepInField(Creature* creature)
{
	...
	if (it.conditionDamage) {
		...
		if (owner) {
			Creature* ownerMaster = owner->getMaster();
