...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags /*= 0*/)
{
	...
	if (player && !diagonalMovement) {
		// try to go up
		Tile* creatureTile = creature->getTile();
		if (currentPos.z != 8 && creatureTile && creatureTile->hasHeight(3)) {
                          ...
                          const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	if (player->hasFlag(PlayerFlag_CanThrowFar)) {
		...
		if (fromCylinderTile && toCylinderTile && playerMoveHangableItem(player->getPosition(), fromCylinderTile->getPosition(), toCylinderTile,
		...
		if (Thing* thing = internalGetThing(player, fromPos, fromStackPos, spriteId, STACKPOS_USEITEM)) {
			...
			if (creatureParent) {
				g_actions->useItemEx(player, fromPos, creature->getPosition(),
				                     static_cast<uint8_t>(creatureParent->getThingIndex(creature)), item, isHotkey,
				                     creature);
			}
		} else {
			player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
